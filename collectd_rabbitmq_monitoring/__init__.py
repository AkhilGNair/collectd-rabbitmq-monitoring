...
def configure(configobj):
    ...
    collectd.info(
        "rabbitmq_monitoring: Connecting to: {}:{} as user:{} password:{}".format(
...
def read(data=None):
    ...
    type_ = "queue_totals"
    for m_instance in ["messages", "messages_ready", "messages_unacknowledged"]:
        if m_instance in overview[type_]:
            metric = collectd.Values()
            metric.meta = {
            ...
            metric.values = [overview[type_][m_instance]]
            metric.dispatch()
            ...
            metric.interval = INTERVAL
            metric.type = "gauge"
            metric.type_instance = "queue_total-{}-rate".format(m_instance)
            metric.values = [overview["queue_totals"]["{}_details".format(m_instance)]["rate"]]
            metric.dispatch()
    ...
    for m_instance in [
        ...
        if m_instance in overview[type_]:
            ...
            metric.values = [overview["message_stats"]["{}_details".format(m_instance)]["rate"]]
            metric.dispatch()
    ...
    for queue_name in queues_to_count:
        ...
        try:
            ...
            collectd.error("Error Opening Queue [{}] details: {}".format(queue_name, err))
    ...
    if timediff > INTERVAL:
        collectd.warning("rabbitmq_monitoring: Took: {} > {}".format(round(timediff, 2), INTERVAL))
